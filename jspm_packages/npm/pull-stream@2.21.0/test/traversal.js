...
function ls_r(start, type) {
  ...
  return type(start, function(dir) {
    ...
    fs.readdir(dir, function(err, ls) {
      ...
      def.resolve(pull.values(ls || []).pipe(pull.map(function(file) {
...
test('widthFirst', function(t) {
  var max = 0,
      didStart;
  pull(ls_r(start, pull.widthFirst), pull.map(function(file) {
    if (file === start)
      didStart = true;
    ...
    if (d > max)
      return max = d, true;
  }), pull.through(console.log), pull.drain(null, function() {
    t.ok(didStart);
    t.end();
  }));
});
test('depthFirst', function(t) {
  var seen = {};
  pull(ls_r(start, pull.depthFirst), pull.through(function(file) {
    if (file != start) {
...
test('leafFirst', function(t) {
  ...
  var expected = {};
  expected[start] = true;
  ls_r(start, pull.leafFirst).pipe(pull.through(function(file) {
    if (file !== start) {
      var dir = path.dirname(file);
      t.ok(!seen[dir]);
      expected[dir] = true;
