...
(function(Buffer) {
  ...
  tape('strings should sort correctly', function(t) {
    var result = sample.map(bytewise.encode).sort(bytewise.compare).map(bytewise.decode);
    t.deepEqual(result, sample);
    t.end();
  });
  tape('nested strings should sort correctly', function(t) {
    ...
    var result = nested.map(bytewise.encode).sort(bytewise.compare).map(bytewise.decode);
