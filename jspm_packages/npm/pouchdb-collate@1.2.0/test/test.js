...
var pouchCollate = require('../lib/index');
...
var verifyLexicalKeysSort = function(keys) {
  ...
  keys.forEach(function(expected, i) {
    ...
    should.equal(actual, expected, 'expect ' + JSON.stringify(actual) + ' is ' + JSON.stringify(expected));
  });
};
...
describe('normalizeKey', function() {
  it('verify key normalizations', function() {
    var normalizations = [[null, null], [NaN, null], [undefined, null], [Infinity, null], [-Infinity, null], ['', ''], ['foo', 'foo'], ['0', '0'], ['1', '1'], [0, 0], [1, 1], [Number.MAX_VALUE, Number.MAX_VALUE], [new Date('1982-11-30T00:00:00.000Z'), '1982-11-30T00:00:00.000Z']];
    normalizations.forEach(function(normalization) {
      ...
      var message = 'check normalization of ' + JSON.stringify(original) + ' to ' + JSON.stringify(expected) + ', got ' + JSON.stringify(normalized);
...
describe('indexableString', function() {
  it('verify intToDecimalForm', function() {
    ...
    utils.intToDecimalForm(-Number.MIN_VALUE).should.equal('0');
    var maxValueStr = '1797693134862316800886484642206468426866682428440286464' + '42228680066046004606080400844208228060084840044686866242482868202680268' + '82040288406280040662242886466688240606642242682208668042640440204020242' + '48802248082808208888442866208026644060866608420408868240026826626668642' + '46642840408646468824200860804260804068888';
  ...
  it('verify toIndexableString()', function() {
    var keys = [null, false, true, -Number.MAX_VALUE, -300, -200, -100, -10, -2.5, -2, -1.5, -1, -0.5, -0.0001, -Number.MIN_VALUE, 0, Number.MIN_VALUE, 0.0001, 0.1, 0.5, 1, 1.5, 2, 3, 10, 15, 100, 200, 300, Number.MAX_VALUE, '', '1', '10', '100', '2', '20', '[]', 'foo', 'mo', 'moe', 'moz', 'mozilla', 'mozilla with a super long string see how far it can go', 'mozzy', [], [null], [null, null], [null, 'foo'], [false], [false, 100], [true], [true, 100], [0], [0, null], [0, 1], [0, ''], [0, 'foo'], ['', ''], ['foo'], ['foo', 1], {}, {'0': null}, {'0': false}, {'0': true}, {'0': 0}, {'0': 1}, {'0': 'bar'}, {'0': 'foo'}, {
      '0': 'foo',
      '1': false
    }, {
      '0': 'foo',
  ...
  it('verify parseIndexableString', function() {
    var keys = [null, false, true, 0, 1, -1, 9, -9, 10, -10, 0.1, -0.1, -0.01, 100, 200, 20, -20, -200, -30, Number.MAX_VALUE, Number.MIN_VALUE, 'foo', '', '\u0000', '\u0001', '\u0002', [1], {foo: true}, {
      foo: 'bar',
      'baz': 'quux',
      foobaz: {
        bar: 'bar',
    ...
    }, {foo: {bar: true}}, [{foo: 'bar'}, {bar: 'baz'}, {}, ['foo', 'bar', 'baz']], [[[['foo']], [], [['bar']]]], -Number.MAX_VALUE, -300, -200, -100, -10, -2.5, -2, -1.5, -1, -0.5, -0.0001, -Number.MIN_VALUE, 0, Number.MIN_VALUE, 0.0001, 0.1, 0.5, 1, 1.5, 2, 3, 10, 15, 100, 200, 300, Number.MAX_VALUE, '', '1', '10', '100', '2', '20', '[]', 'foo', 'mo', 'moe', 'moz', 'mozilla', 'mozilla with a super long string see how far it can go', 'mozzy', [], [null], [null, null], [null, 'foo'], [false], [false, 100], [true], [true, 100], [0], [0, null], [0, 1], [0, ''], [0, 'foo'], ['', ''], ['foo'], ['foo', 1], {}, {'0': null}, {'0': false}, {'0': true}, {'0': 0}, {'0': 1}, {'0': 'bar'}, {'0': 'foo'}, {
      '0': 'foo',
      '1': false
    }, {
      '0': 'foo',
    ...
    keys.forEach(function(key) {
      ...
      JSON.stringify(parseIndexableString(indexableString)).should.equal(JSON.stringify(key), 'check parseIndexableString for key: ' + key + '(indexable string is: ' + indexableString + ')');
    });
  });
