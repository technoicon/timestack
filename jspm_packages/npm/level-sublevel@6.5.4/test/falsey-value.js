...
var falsies = [0, null, false, ''];
var names = ['zero', 'null', 'false', 'emptystring'];
var db = sublevel(level('level-sublevel-falsey', {valueEncoding: 'json'}));
falsies.forEach(function(falsey, i) {
  tape('allow falsey value:' + JSON.stringify(falsey), function(t) {
  ...
  tape('allow falsey value in key', function(t) {
    ...
    sdb.put(falsey, {index: i}, function(err) {
      if (err)
        throw err;
      sdb.createReadStream({gte: falsey}).on('data', function(op) {
