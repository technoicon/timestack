...
test('sublevel - key/value options', function(t) {
  sub.batch(Object.keys(docs).map(function(key) {
    ...
    (function testCreateKeyStream() {
      var results = [];
      sub.createKeyStream().on('data', function(data) {
        results.push(data);
      }).on('end', function() {
    ...
    (function testCreateKeyReadStream() {
      var results = [];
      sub.createReadStream({values: false}).on('data', function(data) {
        results.push(data);
      }).on('end', function() {
        t.deepEqual(results, ['001', '002', '003']);
      });
    ...
    (function testCreateValueStream() {
      var results = [];
      sub.createValueStream({keys: false}).on('data', function(data) {
        results.push(data);
      }).on('end', function() {
    ...
    (function testCreateValueReadStream() {
      var results = [];
      sub.createReadStream({keys: false}).on('data', function(data) {
        results.push(data);
      }).on('end', function() {
        t.deepEqual(results, ['apple', 'orange', 'banana']);
      });
