...
var base = require('../index')(levelup('test-mixed-value-encodings-per-put'));
  ...
  foo.put('foo1', 'foo1-value', {valueEncoding: 'utf8'}, function() {
    bar.put('bar1', {obj: 'ect'}, {valueEncoding: 'json'}, function() {
      ...
      foo.createReadStream({
        ...
        foodata = d;
      }).on('data', console.log.bind(null, '****')).on('error', function(err) {
        fooerr = err;
      }).on('end', function() {
        console.log('END ***********8');
      ...
      var bardata,
          barerr;
      bar.createReadStream({
        start: 'bar1',
        end: 'bar1\xff',
        valueEncoding: 'json'
      }).on('data', console.log.bind(null, '%%%%')).on('data', function(d) {
        bardata = d;
