...
function all(db, cb) {
  ...
  db.createReadStream({end: '\xff\xff'}).on('data', function(ch) {
    obj[ch.key] = ch.value;
...
test('sublevel - batch - chained', function(t) {
  ...
  var b = base.sublevel('B');
  var sum = 0;
  a.batch().put('a', 1).put('b', 2).put('c', 3).put('d', 4).put('e', 5).write(function(err) {
    all(a, function(err, obj) {
      t.notOk(err);
      var keys = Object.keys(obj).join('');
      for (var k in obj) {
...
test('sublevel - prefixed batches - chained', function(t) {
  ...
  var a = base.sublevel('A');
  var b = base.sublevel('B');
  base.batch().put('a', 1).put('b', 2, {prefix: b}).put('c', 3).put('d', 4, {prefix: a}).put('e', 5, {prefix: base}).write(function(err) {
    all(base, function(_, obj) {
      t.deepEqual(obj, {
        'a': '1',
        'c': '3',
...
test('sublevel - prefixed batches on subsection - chained', function(t) {
  ...
  var a = base.sublevel('A');
  var b = base.sublevel('B');
  a.batch().put('a', 1, {prefix: base}).put('b', 2, {prefix: b}).put('c', 3, {prefix: base}).put('d', 4).put('e', 5, {prefix: base}).write(function(err) {
    all(base, function(_, obj) {
      t.deepEqual(obj, {
        'a': '1',
        'c': '3',
...
test('sublevel - prefixed batches on subsection - strings - chained', function(t) {
  ...
  var b = base.sublevel('B');
  var b_c = b.sublevel('C');
  base.batch().put('a', 1).put('b', 2, {prefix: b.prefix()}).put('c', 3).put('d', 4, {prefix: a.prefix()}).put('e', 5).put('f', 6, {prefix: b_c.prefix()}).write(function(err) {
    all(base, function(_, obj) {
      t.deepEqual(obj, {
        'a': '1',
        'c': '3',
...
test('sublevel - delete - chained', function(t) {
  ...
  var b = base.sublevel('B');
  var sum = 0;
  a.batch().put('a', 1).put('b', 2).put('c', 3).put('d', 4).put('e', 5).write();
  a.batch().del('c').del('e').write(function(err) {
    all(a, function(err, obj) {
      t.notOk(err);
      var keys = Object.keys(obj).join('');
      for (var k in obj) {
