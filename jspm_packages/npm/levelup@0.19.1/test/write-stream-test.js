...
buster.testCase('WriteStream', {
  ...
  'test WriteStream with async writes': function(done) {
    this.openTestDatabase(function(db) {
      ...
      ws.on('close', this.verify.bind(this, ws, db, done));
      async.forEachSeries(this.sourceData, function(d, callback) {
        if (d.key % 3) {
          setTimeout(function() {
            ws.write(d);
            callback();
  ...
  'test destroySoon()': function(done) {
    this.openTestDatabase(function(db) {
      ...
      ws.once('ready', ws.destroySoon);
    }.bind(this));
  },
  'test destroy()': function(done) {
    var verify = function(ws, db) {
      async.forEach(this.sourceData, function(data, callback) {
        db.get(data.key, function(err, value) {
          assert(err);
          refute(value);
          callback();
