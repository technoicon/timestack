...
(function(process) {
  var assert = require('referee').assert,
      ...
      fstreamWrite = function(dir, db, callback) {
        fstream.Reader(dir).pipe(db.writeStream({fstreamRoot: dir}).on('close', function() {
      ...
      fstreamRead = function(dir, db, callback) {
        db.readStream({type: 'fstream'}).pipe(new fstream.Writer({
          path: dir + '.out',
          type: 'Directory'
        }).on('close', function() {
          db.close(function(err) {
      ...
      cleanUp = function(dir, callback) {
        async.parallel([rimraf.bind(null, dir + '.out'), rimraf.bind(null, dblocation), mkfiletree.cleanUp], callback);
      };
  ...
  async.waterfall([rimraf.bind(null, dblocation), mkfiletree.makeTemp.bind(null, 'levelup_test_fstream', fixtureFiles), opendb, fstreamWrite, opendb, fstreamRead, function(dir, callback) {
