...
module.exports.extract = function(tarfile, dir, callback) {
  expectedEntries = 0;
  fs.createReadStream(tarfile).pipe(tar.Extract({path: dir})).on('entry', function(entry) {
...
module.exports.fstreamWrite = function(callback) {
  fstream.Reader(datadir).pipe(db.writeStream({fstreamRoot: path.resolve(__dirname)}).on('close', function() {
...
module.exports.verify = function(callback) {
  ...
  db.readStream().on('data', function(data) {
    ...
    entries++;
    fs.createReadStream(path.join(__dirname, data.key)).on('data', function(d) {
      md5sum.update(d);
    }).on('end', function() {
      var fsmd5sum = md5sum.digest('hex');
      assert.equals(dbmd5sum, fsmd5sum, 'MD5 sum compare of ' + data.key + ' failed (' + dbmd5sum + ' != ' + fsmd5sum + ')');
    });
  }).on('end', function() {
